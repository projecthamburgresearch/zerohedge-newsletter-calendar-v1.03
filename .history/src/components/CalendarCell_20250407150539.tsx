...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  useEffect(() => {
    if (!isPublicationsHovered) {
      ...
      setTimeout(updatePubScrollState, 50);
    }
  ...
  useEffect(() => {
    if (!isEventAreaHovered) {
      ...
      setTimeout(updateEventScrollState, 50);
    }
  }, [isEventAreaHovered]);
  ...
  const updatePubScrollState = () => {
    requestAnimationFrame(() => {
      if (pubListRef.current) {
        ...
        setPubCanScroll(scrollHeight > clientHeight + 5);
      }
    });
  };
  ...
  const updateEventScrollState = () => {
    requestAnimationFrame(() => {
      if (eventListRef.current) {
        ...
        setEventCanScroll(scrollHeight > clientHeight + 5);
      }
    });
  };
  ...
  const showPubUpArrow = pubCanScroll && pubScrollTop > 5;
  const showPubDownArrow = pubCanScroll && pubListRef.current && 
    pubScrollTop < (pubListRef.current.scrollHeight - pubListRef.current.clientHeight - 5);
  const showEventUpArrow = eventCanScroll && eventScrollTop > 5;
  const showEventDownArrow = eventCanScroll && eventListRef.current && 
    eventScrollTop < (eventListRef.current.scrollHeight - eventListRef.current.clientHeight - 5);
    ...
    <div className={`p-2 border ${isCurrentDay ? 'bg-brand-yellow-50' : 'bg-white'} h-[100px] relative hover:shadow-lg transition-shadow`}>
      ...
      >
        ...
        {isPublicationsHovered && remainingPublications.length > 0 && (
          <div 
            className="absolute z-30 bottom-full left-0 w-full bg-white shadow-lg rounded border mb-1"
            onMouseEnter={handlePublicationsMouseEnter}
            onMouseLeave={handlePublicationsMouseLeave}
          >
            <div className="relative p-1">
              <button 
                className={`absolute top-1 left-1 right-1 w-auto bg-gradient-to-b from-white via-white/90 to-transparent h-6 flex justify-center items-center z-20 rounded-t transition-opacity duration-150 ${showPubUpArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
              ...
              <button 
                className={`absolute bottom-1 left-1 right-1 w-auto bg-gradient-to-t from-white via-white/90 to-transparent h-6 flex justify-center items-center z-20 rounded-b transition-opacity duration-150 ${showPubDownArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      ...
      >
        ...
        {isEventAreaHovered && remainingEvents.length > 0 && (
          <div 
            className="absolute z-30 top-full left-0 w-full bg-white shadow-lg rounded border mt-1"
            onMouseEnter={handleEventAreaMouseEnter}
            onMouseLeave={handleEventAreaMouseLeave}
          >
            <div className="relative p-1">
              <button 
                className={`absolute top-1 left-1 right-1 w-auto bg-gradient-to-b from-white via-white/90 to-transparent h-6 flex justify-center items-center z-20 rounded-t transition-opacity duration-150 ${showEventUpArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
              ...
              <button 
                className={`absolute bottom-1 left-1 right-1 w-auto bg-gradient-to-t from-white via-white/90 to-transparent h-6 flex justify-center items-center z-20 rounded-b transition-opacity duration-150 ${showEventDownArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
