...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  // Publications carousel navigation - one item at a time
  const goToPubNext = () => {
  ...
  // Event carousel navigation - one item at a time
  const goToEventNext = () => {
  ...
  const startPubAutoScroll = (direction: 'top' | 'bottom') => {
    ...
    const scroll = () => {
      ...
      if (canContinue) {
        pubScrollTimer.current = setTimeout(scroll, 500); // 0.5 second delay between items
      }
    };

    // Start the first scroll with a small initial delay
    pubScrollTimer.current = setTimeout(scroll, 100);
  };
  ...
  const startEventAutoScroll = (direction: 'top' | 'bottom') => {
    ...
    const scroll = () => {
      ...
      if (canContinue) {
        eventScrollTimer.current = setTimeout(scroll, 500); // 0.5 second delay between items
      }
    };

    // Start the first scroll with a small initial delay
    eventScrollTimer.current = setTimeout(scroll, 100);
  };
  ...
  // Calculate visible items (for scrolling calculation)
  const VISIBLE_ITEMS = 3; // Number of items visible in the carousel at once
      ...
      >
        ...
        {isPublicationsHovered && remainingPublications.length > 0 && (
          ...
          >
            ...
            {/* Carousel container */}
            <div className="relative overflow-hidden max-h-[150px] py-1">
              <div 
                ref={pubCarouselRef}
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{
                  transform: `translateY(-${pubCarouselIndex * 34}px)`,
                  paddingTop: remainingPublications.length > VISIBLE_ITEMS ? '8px' : '0',
                  paddingBottom: remainingPublications.length > VISIBLE_ITEMS ? '8px' : '0'
                }}
              >
                {remainingPublications.map((pub, index) => (
                  <div
                    key={`${pub.source}-${pub.seriesTitle}-${index}`}
                    onClick={() => onPublicationClick(pub)}
                    className={`text-xs p-2 rounded cursor-pointer hover:bg-brand-yellow-100 transition-all mb-2 h-[34px] ${
                      pubCarouselIndex === index ? 'bg-brand-yellow-50' : ''
                    }`}
                  >
                    <span className="font-semibold block truncate">{pub.source}</span>
                    <span className="text-gray-600 line-clamp-1 text-[10px] block">{pub.seriesTitle}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Navigation dots */}
            {remainingPublications.length > VISIBLE_ITEMS && (
              <div className="flex justify-center mt-2 gap-1">
                {Array.from({ length: Math.min(5, remainingPublications.length) }).map((_, index) => {
                  // Calculate the dot position relative to total items
                  const position = Math.floor(index * (remainingPublications.length / 5));
                  const isActive = pubCarouselIndex === position || 
                    (index === 4 && pubCarouselIndex >= position);
                  
                  return (
                    <button
                      key={index}
                      onClick={() => setPubCarouselIndex(position)}
                      className={`h-1.5 rounded-full transition-all ${
                        isActive ? 'w-4 bg-brand-yellow-300' : 'w-1.5 bg-gray-300 hover:bg-gray-400'
                      }`}
          ...
          >
            ...
            <div className="relative overflow-hidden max-h-[150px] py-1">
              <div 
                ...
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{
                  transform: `translateY(-${eventCarouselIndex * 34}px)`,
                  paddingTop: remainingEvents.length > VISIBLE_ITEMS ? '8px' : '0',
                  paddingBottom: remainingEvents.length > VISIBLE_ITEMS ? '8px' : '0'
                }}
              >
                {remainingEvents.map((event, index) => (
                  <div
                    ...
                    className={`
                      text-xs p-2 rounded cursor-pointer transition-all mb-2 h-[34px]
                      ${getEventTypeColor(event.type)}
                      ${eventCarouselIndex === index ? 'opacity-100' : 'opacity-90'}
                    `}
                  >
                    <span className="block truncate">{event.title}</span>
                    <span className="text-gray-700 text-[10px] block">
                      {event.type}
                    </span>
                  </div>
                ))}
            ...
            {/* Navigation dots */}
            {remainingEvents.length > VISIBLE_ITEMS && (
              <div className="flex justify-center mt-2 gap-1">
                {Array.from({ length: Math.min(5, remainingEvents.length) }).map((_, index) => {
                  // Calculate the dot position relative to total items
                  const position = Math.floor(index * (remainingEvents.length / 5));
                  const isActive = eventCarouselIndex === position ||
                    (index === 4 && eventCarouselIndex >= position);
                    
                  return (
                    <button
                      key={index}
                      onClick={() => setEventCarouselIndex(position)}
                      className={`h-1.5 rounded-full transition-all ${
                        isActive ? 'w-4 bg-gray-500' : 'w-1.5 bg-gray-300 hover:bg-gray-400'
                      }`}
                      aria-label={`Go to event position ${position + 1}`}
                    />
                  );
                })}
              </div>
