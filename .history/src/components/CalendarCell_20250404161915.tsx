      ...
      >
        ...
        {isPublicationsHovered && remainingPublications.length > 0 && (
          <div 
            ...
            onMouseLeave={handlePublicationsMouseLeave}
          >
            {/* Edge hover zones for auto-scrolling - Top */}
            {pubCarouselIndex > 0 && (
              <div 
                className="absolute top-0 left-0 right-0 h-6 z-30 bg-gradient-to-b from-white/80 to-transparent flex justify-center cursor-pointer"
                onMouseEnter={() => startPubAutoScroll('up')}
            ...
            {/* Edge hover zones for auto-scrolling - Bottom */}
            {pubCarouselIndex < pubPages - 1 && (
              <div 
                className="absolute bottom-0 left-0 right-0 h-6 z-30 bg-gradient-to-t from-white/80 to-transparent flex justify-center cursor-pointer"
                onMouseEnter={() => startPubAutoScroll('down')}
                onMouseLeave={stopPubAutoScroll}
                onClick={goToPubNext}
              >
                <div className="text-brand-yellow-400 opacity-70 hover:opacity-100">
            ...
            {/* Carousel container */}
            <div className="relative overflow-hidden" style={{ height: `${Math.min(remainingPublications.length, 3) * 48}px` }}>
              <div 
                ref={pubCarouselRef}
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{ transform: `translateY(-${pubCarouselIndex * 100}%)` }}
              >
                {/* Group publications into sets of 3 */}
                {Array.from({ length: pubPages }).map((_, pageIndex) => (
                  <div key={pageIndex} className="flex flex-col w-full min-w-full gap-1 min-h-full">
                    {remainingPublications.slice(pageIndex * 3, pageIndex * 3 + 3).map((pub, index) => (
                      <div
                        key={`${pub.source}-${pub.seriesTitle}-${pageIndex}-${index}`}
                        onClick={() => onPublicationClick(pub)}
                        className="text-xs p-2 rounded cursor-pointer hover:bg-brand-yellow-100 transition-all mb-1 last:mb-0"
                      >
                        <div className="flex justify-between items-center">
                          <span className="font-semibold">{pub.source}</span>
                          <span className="text-[9px] text-gray-500">{pub.newsletterType || 'Publication'}</span>
                        </div>
                        <div className="text-gray-600 text-[10px] mt-0.5 line-clamp-2">{pub.seriesTitle}</div>
                      </div>
                    ))}
                  </div>
                ))}
        ...
        {isEventAreaHovered && remainingEvents.length > 0 && (
          ...
          >
            ...
            {eventCarouselIndex > 0 && (
              <div 
                className="absolute top-0 left-0 right-0 h-6 z-30 bg-gradient-to-b from-white/80 to-transparent flex justify-center cursor-pointer"
                onMouseEnter={() => startEventAutoScroll('up')}
            ...
            {eventCarouselIndex < eventPages - 1 && (
              <div 
                className="absolute bottom-0 left-0 right-0 h-6 z-30 bg-gradient-to-t from-white/80 to-transparent flex justify-center cursor-pointer"
                onMouseEnter={() => startEventAutoScroll('down')}
                onMouseLeave={stopEventAutoScroll}
            ...
            {/* Carousel container */}
            <div className="relative overflow-hidden" style={{ height: `${Math.min(remainingEvents.length, 3) * 48}px` }}>
              <div 
                ref={eventCarouselRef}
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{ transform: `translateY(-${eventCarouselIndex * 100}%)` }}
              >
                  ...
                  <div key={pageIndex} className="flex flex-col w-full min-w-full gap-1 min-h-full">
                    {remainingEvents.slice(pageIndex * 3, pageIndex * 3 + 3).map((event, index) => (
                      <div
                        ...
                        className={`
                          text-xs p-2 rounded cursor-pointer transition-all mb-1 last:mb-0
                          ${getEventTypeColor(event.type)}
                        `}
                      >
                        <div className="flex justify-between items-center">
                          <span className="font-semibold">{event.title}</span>
                          <span className="text-[9px] text-gray-700">{event.type}</span>
                        </div>
                        <div className="text-gray-700 text-[10px] mt-0.5 line-clamp-1">{event.description || ''}</div>
                      </div>
                    ))}
                  </div>
                ))}
