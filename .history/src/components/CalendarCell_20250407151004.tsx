...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const updatePubScrollState = () => {
    requestAnimationFrame(() => {
      if (pubListRef.current) {
        ...
        console.log('[Pub Scroll Update]', { scrollTop, scrollHeight, clientHeight, canScroll: scrollHeight > clientHeight + 1 });
  ...
  const updateEventScrollState = () => {
    requestAnimationFrame(() => {
      if (eventListRef.current) {
        ...
        console.log('[Event Scroll Update]', { scrollTop, scrollHeight, clientHeight, canScroll: scrollHeight > clientHeight + 1 });
  ...
  const showPubUpArrow = pubCanScroll && pubScrollTop > 1;
  ...
  const showEventUpArrow = eventCanScroll && eventScrollTop > 1;
        ...
        {isPublicationsHovered && remainingPublications.length > 0 && (
          ...
          >
            <div className="relative p-1">
              <button 
                className={`absolute top-1 left-1 right-1 w-auto bg-red-500 h-6 flex justify-center items-center z-20 rounded-t transition-opacity duration-150 ${showPubUpArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
              ...
              <button 
                className={`absolute bottom-1 left-1 right-1 w-auto bg-red-500 h-6 flex justify-center items-center z-20 rounded-b transition-opacity duration-150 ${showPubDownArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      ...
      >
        ...
        {isEventAreaHovered && remainingEvents.length > 0 && (
          ...
          >
            <div className="relative p-1">
              <button 
                className={`absolute top-1 left-1 right-1 w-auto bg-red-500 h-6 flex justify-center items-center z-20 rounded-t transition-opacity duration-150 ${showEventUpArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
              ...
              <button 
                className={`absolute bottom-1 left-1 right-1 w-auto bg-red-500 h-6 flex justify-center items-center z-20 rounded-b transition-opacity duration-150 ${showEventDownArrow ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
