...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const startPubAutoScroll = (direction: 'top' | 'bottom') => {
    ...
    const scroll = () => {
      ...
      // Always continue scrolling with a consistent delay
      // Use requestAnimationFrame to schedule next scroll after state update
      requestAnimationFrame(() => {
        pubScrollTimer.current = setTimeout(scroll, 800); // Continue scrolling every 800ms
      });
    };

    // First scroll after a short delay
    pubScrollTimer.current = setTimeout(scroll, 300);
  };
  ...
  const startEventAutoScroll = (direction: 'top' | 'bottom') => {
    ...
    // First scroll after a short delay
    eventScrollTimer.current = setTimeout(scroll, 300);
  };
