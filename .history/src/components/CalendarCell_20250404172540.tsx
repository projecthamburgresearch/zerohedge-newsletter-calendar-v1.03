...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const pubScrollIntervalId = useRef<NodeJS.Timeout | null>(null);
  const eventScrollIntervalId = useRef<NodeJS.Timeout | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      if (pubScrollIntervalId.current) clearInterval(pubScrollIntervalId.current);
      if (eventScrollIntervalId.current) clearInterval(eventScrollIntervalId.current);
    };
  }, []);

  // Reset carousel index and clear intervals when hover state changes
  useEffect(() => {
    if (!isPublicationsHovered) {
      setPubCarouselIndex(0);
      if (pubScrollIntervalId.current) {
        clearInterval(pubScrollIntervalId.current);
        pubScrollIntervalId.current = null;
      }
    }
    
    if (!isEventAreaHovered) {
      setEventCarouselIndex(0);
      if (eventScrollIntervalId.current) {
        clearInterval(eventScrollIntervalId.current);
        eventScrollIntervalId.current = null;
      }
    }
  ...
  // Continuous edge scrolling for publications using setInterval
  const startPubAutoScroll = (direction: 'top' | 'bottom') => {
    // Clear any existing interval
    stopPubAutoScroll(); 
    console.log('Starting pub auto-scroll interval:', direction);

    // Perform the first scroll immediately
    if (direction === 'top') {
      goToPubPrev();
    } else {
      goToPubNext();
    ...
    // Start interval for subsequent scrolls
    pubScrollIntervalId.current = setInterval(() => {
      console.log('Pub auto-scroll tick:', direction);
      if (direction === 'top') {
        goToPubPrev();
      } else {
        goToPubNext();
  ...
  // Stop pub auto-scrolling interval
  const stopPubAutoScroll = () => {
    if (pubScrollIntervalId.current) {
      console.log('Stopping pub auto-scroll interval');
      clearInterval(pubScrollIntervalId.current);
      pubScrollIntervalId.current = null;
    }
  };

  // Continuous edge scrolling for events using setInterval
  const startEventAutoScroll = (direction: 'top' | 'bottom') => {
    // Clear any existing interval
    stopEventAutoScroll();
    console.log('Starting event auto-scroll interval:', direction);
    ...
    // Start interval for subsequent scrolls
    eventScrollIntervalId.current = setInterval(() => {
      console.log('Event auto-scroll tick:', direction);
      if (direction === 'top') {
        goToEventPrev();
      } else {
        goToEventNext();
  ...
  // Stop event auto-scrolling interval
  const stopEventAutoScroll = () => {
    if (eventScrollIntervalId.current) {
      console.log('Stopping event auto-scroll interval');
      clearInterval(eventScrollIntervalId.current);
      eventScrollIntervalId.current = null;
    }
  };
      ...
      >
        ...
        {isPublicationsHovered && remainingPublications.length > 0 && (
          ...
          >
            ...
            {/* Carousel container */}
            <div className="relative overflow-hidden max-h-[200px] py-2 px-1">
              <div 
                ref={pubCarouselRef}
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{ 
                  transform: `translateY(${-pubCarouselIndex * 66}px)` // Scroll by 66px (item height + padding + margin)
                }}
              >
                {remainingPublications.map((pub, index) => (
                  <div
                    key={`${pub.source}-${pub.seriesTitle}-${index}`}
                    onClick={() => onPublicationClick(pub)}
                    className="text-xs p-2 rounded cursor-pointer hover:bg-brand-yellow-100 transition-all mb-3 bg-white"
                    style={{ height: '38px' }} // Explicit height for content area
                  >
          ...
          >
            ...
            <div className="relative overflow-hidden max-h-[200px] py-2 px-1">
              <div 
                ...
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{ 
                  transform: `translateY(${-eventCarouselIndex * 66}px)` // Scroll by 66px (item height + padding + margin)
                }}
              >
                {remainingEvents.map((event, index) => (
                  <div
                    ...
                    className={`
                      text-xs p-2 rounded cursor-pointer transition-all mb-3
