...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const goToPubNext = () => {
    ...
    // Circular navigation - if at last page, go to first
  ...
  const goToPubPrev = () => {
    ...
    // Circular navigation - if at first page, go to last
  ...
  const pubPages = Math.max(1, Math.ceil(remainingPublications.length / ITEMS_PER_PAGE));
  const eventPages = Math.max(1, Math.ceil(remainingEvents.length / ITEMS_PER_PAGE));
          ...
          >
            ...
            <div className="relative overflow-hidden max-h-[150px] py-1">
              <div 
                ...
                style={{ transform: `translateY(-${pubCarouselIndex * (100 / pubPages)}%)` }}
              >
                {remainingPublications.map((pub, index) => (
                  <div
                    key={`${pub.source}-${pub.seriesTitle}-${index}`}
                    onClick={() => onPublicationClick(pub)}
                    className="text-xs p-2 rounded cursor-pointer hover:bg-brand-yellow-100 transition-all mb-2 last:mb-0"
                  >
          ...
          >
            ...
            <div className="relative overflow-hidden max-h-[150px] py-1">
              <div 
                ...
                className="flex flex-col transition-transform duration-300 ease-in-out"
                style={{ transform: `translateY(-${eventCarouselIndex * (100 / eventPages)}%)` }}
              >
                {remainingEvents.map((event, index) => (
                  <div
                    ...
                    className={`
                      text-xs p-2 rounded cursor-pointer transition-all mb-2 last:mb-0
