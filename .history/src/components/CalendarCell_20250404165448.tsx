            ...
            <div className="relative overflow-hidden max-h-[180px]">
              ...
              >
                ...
                {Array.from({ length: pubPages }).map((_, pageIndex) => (
                  <div key={pageIndex} className="flex flex-col w-full min-w-full gap-1 min-h-[160px]">
                    {remainingPublications.slice(pageIndex * 3, pageIndex * 3 + 3).map((pub, index) => (
                      <div
                        key={`${pub.source}-${pub.seriesTitle}-${pageIndex}-${index}`}
                        onClick={() => onPublicationClick(pub)}
                        className="text-xs p-2 rounded cursor-pointer hover:bg-brand-yellow-100 transition-all w-full mb-1"
                      >
                  ...
                  <div key={pageIndex} className="flex flex-col w-full min-w-full gap-1 min-h-[160px]">
                    {remainingEvents.slice(pageIndex * 3, pageIndex * 3 + 3).map((event, index) => (
                      <div
                        ...
                        className={`
                          text-xs p-2 rounded cursor-pointer transition-all w-full mb-1
