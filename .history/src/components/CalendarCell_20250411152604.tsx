...
import { getAbbreviatedSource, formatDate } from '@/utils/dateUtils';
...
export const CalendarCell: React.FC<CalendarCellProps> = ({
  ...
  const formattedDayDate = day.date.replace(/-/g, '');

  // Filter publications for this day - ensure proper typing first
  const dayPublications = publications.filter(pub => {
    if (!pub.ReleaseDate) return false;
    // Explicitly check if this is a Publication type to avoid confusion
    if (!('PublicationTitle' in pub)) return false;
    
    const pubDate = pub.ReleaseDate;
    return pubDate.substring(0, 8) === formattedDayDate;
  });

  // Filter events for this day - handle multi-day events properly
  const dayEvents = events.filter(event => {
    if (!event.ReleaseDate) return false;
    // Explicitly check if this is an Event type to avoid confusion
    if (!('EventTitle' in event)) return false;
    
    // Convert to numeric format for easier comparison
    const dateNum = parseInt(formattedDayDate);
    const startNum = parseInt(event.ReleaseDate);
    const endNum = event.EventEndDate ? parseInt(event.EventEndDate) : startNum;
    
    // Include event if this day is within the date range
    return dateNum >= startNum && dateNum <= endNum;
  });

  // Ensure we're displaying publications and events in correct places
  ...
  return (
    <div className={`p-2 border ${isCurrentDay ? 'bg-brand-yellow-50' : 'bg-white'} h-[100px] relative hover:shadow-lg transition-shadow`}>
      ...
      >
        ...
        {/* Only display Publication items in the publications area */}
      ...
      >
        {/* Only display Event items in the events area */}
        {currentEvent && (
          <div
            onClick={() => handleEventClick(currentEvent)}
            className="text-xs p-1 rounded cursor-pointer whitespace-nowrap overflow-hidden flex items-center justify-between bg-gray-50 hover:bg-gray-100 border border-gray-200"
            title={`${currentEvent.EventTitle}${
              currentEvent.ReleaseDate !== currentEvent.EventEndDate && 
            ...
            <div className="flex-1 overflow-hidden pr-1 marquee-text">
              <div className="inline-flex scrollable-text">
                ...
                {currentEvent.ReleaseDate !== currentEvent.EventEndDate && currentEvent.EventEndDate && (
                  <span className="whitespace-nowrap text-gray-500 ml-1 text-xs">
                    ({formatDate(currentEvent.ReleaseDate).substring(5)} - {formatDate(currentEvent.EventEndDate).substring(5)})
                  </span>
                )}
              </div>
            </div>
                ...
                <div className="space-y-1 px-1" data-event-list="true">
                  {remainingEvents.map((event, index) => (
                    <div
                      ...
                      className="group text-xs p-2 rounded cursor-pointer bg-gray-50 hover:bg-gray-100 border border-gray-200"
                    >
